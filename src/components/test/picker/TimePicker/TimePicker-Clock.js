...
export default mixins(
  ...
  /* eslint-enable indent */ Colorable,
  Themeable
  /* @vue/component */
).extend({
  ...
  methods: {
    ...
    genHand() {
      ...
      const angle =
        this.rotate + this.degreesPerUnit * (this.displayedValue - this.min)
    ...
    getPosition(value) {
      ...
      return {
        x:
          Math.sin((value - this.min) * this.degrees + rotateRadians) *
          this.handScale(value),
        y:
          -Math.cos((value - this.min) * this.degrees + rotateRadians) *
          this.handScale(value)
      }
    },
    onMouseDown(e) {
    ...
    onDragMove(e) {
      ...
      const handAngle =
        Math.round(this.angle(center, coords) - this.rotate + 360) % 360
      const insideClick =
        this.double &&
