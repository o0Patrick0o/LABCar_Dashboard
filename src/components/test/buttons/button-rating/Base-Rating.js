...
export const VRating = genericComponent()({
  ...
  setup(props, { slots }) {
    ...
    const normalizedValue = computed(() =>
      clamp(parseFloat(rating.value), 0, +props.length)
    )

    const range = computed(() => createRange(Number(props.length), 1))
    const increments = computed(() =>
      range.value.flatMap(v => (props.halfIncrements ? [v - 0.5, v] : [v]))
    )
    const hoverIndex = shallowRef(-1)

    const itemState = computed(() =>
      increments.value.map(value => {
    ...
    const eventState = computed(() =>
      [0, ...increments.value].map(value => {
        ...
        function onClick() {
          ...
          rating.value =
            normalizedValue.value === value && props.clearable ? 0 : value
        }
    ...
    function VRatingItem({ value, index, showStar = true }) {
      const { onMouseenter, onMouseleave, onClick } = eventState.value[
        index + 1
      ]
