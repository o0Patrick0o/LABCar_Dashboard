<template>
  <md-menu class="md-select" :class="{ 'md-disabled': disabled }" :md-close-on-select="false" :active="showSelect" :md-offset-x="offset.x" :md-offset-y="offset.y" :md-dense="mdDense" @md-closed="onClose">
    <md-input class="md-input md-select-value" v-model="MdSelect.label" ref="input" readonly :disabled="disabled" :required="required" :placeholder="placeholder" v-on="inputListeners" v-bind="attrs"
      @focus.prevent="onFocus" @blur.prevent="removeHighlight" @click="openSelect" @keydown.down="openSelect" @keydown.enter="openSelect" @keydown.space="openSelect"  />
    <md-drop-down-icon @click.native="openSelect" />

    <keep-alive>
      <md-menu-content ref="menu" class="md-select-menu" :md-content-class="mdClass" :style="menuStyles" @enter="onMenuEnter">
