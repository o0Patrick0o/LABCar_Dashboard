<template>
  <div class="relative inline-flex p-1 shadow-xl border border-gray-300 rounded">
    
    <button ref="trigger" style="width: 36; height: 36;"
      class="bg-white border-slate-200 hover:border-slate-600 text-slate-500 hover:text-slate-600 rounded w-9 h-9"
      aria-haspopup="true" @click.prevent="dropdownOpen = !dropdownOpen" :aria-expanded="dropdownOpen">
      <span class="sr-only">Filter</span><wbr />
      <font-awesome-icon icon="fa-solid fa-filter"  />
    </button>

    <transition enter-active-class="transition ease-out duration-200 transform" enter-from-class="opacity-0 -translate-y-2" enter-to-class="opacity-100 translate-y-0"
